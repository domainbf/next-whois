...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // Get all active reminders with a known expiration date
  ...
  for (const reminder of reminders) {
    ...
    // Auto-cancel: domain has been expired for more than 45 days (likely redeemed or deleted)
    if (daysLeft < -45) {
      await db.query(
    ...
    // Check each threshold in descending order (60, 30, 10, 5, 1)
    for (const threshold of THRESHOLDS) {
      if (daysLeft <= threshold && !sentThresholds.includes(threshold)) {
        ...
        results.sent++;
        break; // Only send one email per run per reminder
      }
    }

    // If all thresholds sent and domain is expired, auto-cancel
